...
def _pandoc_docx_toolchain_impl(ctx):
    ...
    template_variables = platform_common.TemplateVariableInfo({
        "pandoc_docx_BIN": target_tool_path,
    })
    default = DefaultInfo(
        files = depset(tool_files),
        runfiles = ctx.runfiles(files = tool_files),
    )
    pandocdocxinfo = PandocDocxInfo(
...
pandoc_docx_toolchain = rule(
    implementation = _pandoc_docx_toolchain_impl,
    attrs = {
    ...
    doc = """Defines a pandoc_docx compiler/runtime toolchain.
